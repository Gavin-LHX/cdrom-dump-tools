...
- MusicBrainz 返回多个发行版本时弹出候选列表，可按艺术家、专辑、日期、地区和碟号选择；高级设置仍可用非零序号强制覆盖。
- 转换前检查路径与参数，并以参数数组启动 PowerShell，不通过 `cmd.exe` 拼接执行命令。
- 运行日志与命令预览使用独立页签；日志可复制、清空或切换“自动跟随最新日志”。用户向上阅读时会保留滚动位置，空的定时刷新也不会再反复把滚动条拉到底部。命令可一键复制、自动换行、只保留纵向滚动条，API Key 不会进入预览内容。
...
4. 将“自定义输出目录”留空以自动命名，或指定一个尚不存在的完整目标目录；FFmpeg、`.env`、强制候选序号和 User-Agent 位于“高级设置”。候选序号保持 `0` 时，多版本光盘会在识别后弹窗询问。
5. 开始转换，在进度区查看当前在线查询阶段、曲目 `X/Y` 和耗时；若出现 MusicBrainz 候选窗口，选择光盘实际版本后即可继续。“运行日志”“命令预览”页签可随时切换。转换成功后可用“打开输出目录”进入实际结果目录。
